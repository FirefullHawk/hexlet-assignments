...
public class ProductsController {
    ...
    ProductDTO update(@RequestBody ProductUpdateDTO productData, @PathVariable Long id) {
        Product productFromBase =  productRepository
                               .findById(id)
                               .orElseThrow(
                                   () -> new ResourceNotFoundException("Product with id " + id + " not found"));
        productFromBase.setPrice(productData.getPrice());
        productFromBase.setTitle(productData.getTitle());

        return toDTO(productRepository.save(productFromBase));
    }
    // END
