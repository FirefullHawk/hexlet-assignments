...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> firstNameValidator = ctx.formParamAsClass("firstName", String.class)
                .check(it -> !it.isEmpty(), "Поле не должно быть пустым!");
        ...
        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(it -> EmailValidator.getInstance().isValid(it), "Email введён с ошибкой!");

        Validator<String> passwordNameValidator = ctx.formParamAsClass("password", String.class)
                .check(StringUtils::isNumeric, "Пароль должен содержать только цифры!")
                .check(it -> it.length() > 4, "Длина пароля должна быть не менее 4 символов!");
